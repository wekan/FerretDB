...
- Integration test setup: stop flooding the output with `traces export: Post "http://127.0.0.1:4318/v1/traces": connection refused` when no OTel collector is running. The OTel trace exporter is optional tooling (FerretDB's task runner starts a collector in Docker; `build.sh` does not), but it was created unconditionally and retried forever. The setup now probes 127.0.0.1:4318 once and skips trace exporting with a single log line when nothing is listening. Thanks to xet7.
- Add Go table tests for the SQLite filter pushdown (`internal/backends/sqlite/query_test.go`): which strings are pushdown-safe (Go-JSON vs SQLite `->` serialization), exact WHERE clauses and args for `_id` and top-level equality filters (including the array-containment range arm and compound filters), and negative cases proving dotted paths, operators, non-string values and unsafe strings stay with the in-Go filter. Thanks to xet7.
