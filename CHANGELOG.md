...
- Self-throttle command: add a custom `throttle` command so a client that shares the host with FerretDB can ask it what it is doing and to slow down when the host CPU is high (a client cannot pause FerretDB's internal work from the outside). The command returns `commandsProcessed` — a running count of handled commands, an activity/"how busy" signal a client can log — and enables a **self-expiring throttle**: while active, every command pauses `slowDownMs` (clamped to `[0, 1000]`, default 5) before it runs, for `durationMs` (clamped to `[0, 300000]`, default 2000), which lowers FerretDB's CPU use and yields time to other software on the host. The throttle is a lock-free atomic state applied in the command dispatch path (`internal/handler/commands.go`), skipping the throttle command itself and the cheap health/handshake commands (`hello`/`ismaster`/`ping`) so a client's monitoring and control are never delayed; because it self-expires (max 5 min) a crashed or disconnected client can never leave FerretDB permanently slow. New files `internal/handler/throttle.go` and `internal/handler/msg_throttle.go`; covered by `internal/handler/throttle_test.go` (set/clamp/expiry, command counting, no block when inactive, and prompt return on a cancelled context) by @xet7. Thanks to xet7.
