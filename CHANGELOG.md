...
- WeKan CPU governor: add a custom `wekanThrottle` command so WeKan can ask FerretDB what it is doing and to slow down when the host CPU is high (WeKan and FerretDB share the machine, and WeKan cannot pause FerretDB's internal work from the outside). The command returns `commandsProcessed` — a running count of handled commands, an activity/"how busy" signal WeKan writes to its Admin Panel → Problems → CPU usage log — and enables a **self-expiring throttle**: while active, every command pauses `slowDownMs` (clamped to `[0, 1000]`, default 5) before it runs, for `durationMs` (clamped to `[0, 300000]`, default 2000), which lowers FerretDB's CPU use and yields time to other software. The throttle is a lock-free atomic state applied in the command dispatch path (`internal/handler/commands.go`), skipping the throttle command itself and the cheap health/handshake commands (`hello`/`ismaster`/`ping`) so WeKan's monitoring and control are never delayed; because it self-expires (max 5 min) a WeKan crash can never leave FerretDB permanently slow. WeKan renews it every sample while CPU stays high and calls it with a zero duration (resume) when CPU recovers. New files `internal/handler/wekanthrottle.go` and `internal/handler/msg_wekanthrottle.go`; covered by `internal/handler/wekanthrottle_test.go` (set/clamp/expiry, command counting, no block when inactive, and prompt return on a cancelled context) by @xet7. Thanks to xet7.
